...
with gr.Blocks() as demo:
    gr.Markdown("# 문장 분리 데모")
    gr.Markdown("띄어쓰기와 문장 부호가 없는 텍스트를 입력하면, 문장 단위로 분리된 결과를 출력합니다.")
    gr.Markdown("모델: HuggingFaceTB/SmolLM3-3B")
    gr.Markdown("---")
    gr.Markdown("예시 문장")
    ...
    with gr.Row():
        with gr.Column():
            input_text = gr.Textbox(label="입력 텍스트", lines=10, placeholder="띄어쓰기와 문장 부호가 없는 텍스트를 입력하세요.")
            threshold_punct = gr.Slider(0.0, 0.1, value=0.03, step=0.01, label="문장 부호 임계값")
            # threshold_space = gr.Slider(0.0, 0.1, value=0.05, step=0.01, label="띄어쓰기 임계값")
            submit_button = gr.Button("분리 시작")
        
        with gr.Column():
            output_text = gr.Textbox(label="분리된 문장", lines=10)
        
        submit_button.click(
            fn=lambda text, tp: "\n\n\n".join(cleaner.clean(text, threshold_punct=tp)),
            inputs=[input_text, threshold_punct],
            outputs=output_text
        )
